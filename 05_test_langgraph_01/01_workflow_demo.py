...
def rag_node(state: MyState) -> MyState | dict:
    query = state["query"]
...
# web处理节点
def web_search_node(state: MyState) -> MyState | dict:
    query = state["query"]
...
# 汇总节点
def final_answer_node(state: MyState) -> MyState | dict:
...
# 7. 启动工作流
state = {"query": "万用表如何使用？"}
final_state = compile_graph.invoke(state)
# 8. 输出结果
# 格式化json
